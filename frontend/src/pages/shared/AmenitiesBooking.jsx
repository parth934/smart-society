...
const AmenitiesBooking = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch Facilities
        const facRes = await axios.get('https://smart-society-rr5e.onrender.com/api/amenities');
        setFacilities(facRes.data);

        // Fetch Bookings
        const bkgRes = await axios.get('https://smart-society-rr5e.onrender.com/api/amenities/bookings/all');
  ...
  const handleSaveFacility = async (e) => {
    ...
    try {
      if (editingFacilityId) {
        const response = await axios.put(`https://smart-society-rr5e.onrender.com/api/amenities/${editingFacilityId}`, facilityForm);
        setFacilities(facilities.map(f => f._id === editingFacilityId ? response.data : f));
      } else {
        const response = await axios.post('https://smart-society-rr5e.onrender.com/api/amenities', facilityForm);
  ...
  const handleConfirmBooking = async (e) => {
    ...
    try {
      const response = await axios.post('https://smart-society-rr5e.onrender.com/api/amenities/bookings/new', newBooking);
  ...
  const handleStatusUpdate = async (id, newStatus) => {
    try {
      const response = await axios.put(`https://smart-society-rr5e.onrender.com/api/amenities/bookings/${id}`, { status: newStatus });
