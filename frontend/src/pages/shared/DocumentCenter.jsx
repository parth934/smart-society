...
const DocumentCenter = () => {
  ...
  useEffect(() => {
    const fetchDocs = async () => {
      try {
        const response = await axios.get('https://smart-society-rr5e.onrender.com/api/documents');
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        const response = await axios.put(`https://smart-society-rr5e.onrender.com/api/documents/${editingId}`, formData);
        setDocuments(documents.map(d => d._id === editingId ? response.data : d));
      } else {
        const response = await axios.post('https://smart-society-rr5e.onrender.com/api/documents', formData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this document link?")) {
      try {
        await axios.delete(`https://smart-society-rr5e.onrender.com/api/documents/${id}`);
