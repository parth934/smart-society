...
const ServicesDirectory = () => {
  ...
  const handleAddCategory = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/services', { category: newCategoryName });
  ...
  const handleAddContact = async (e, categoryId) => {
    ...
    try {
      const response = await axios.post(`http://localhost:5000/api/services/${categoryId}/contacts`, newContact);
  ...
  const handleDeleteContact = async (categoryId, contactId) => {
    if (!isAdmin) return;
    try {
      const response = await axios.delete(`http://localhost:5000/api/services/${categoryId}/contacts/${contactId}`);
      setServices(services.map(srv => srv._id === categoryId ? response.data : srv));
    } catch (error) {
