...
const AdminAmenities = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        // UPDATE
        const response = await axios.put(`http://localhost:5000/api/amenities/${editingId}`, formData);
        setAmenities(amenities.map(a => a._id === editingId ? response.data : a));
      } else {
        // CREATE
        const response = await axios.post('http://localhost:5000/api/amenities', formData);
