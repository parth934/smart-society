...
const SecurityDashboard = () => {
  ...
  const handleAddVisitor = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/visitors', newVisitor);
  ...
  const handleMarkExit = async (id) => {
    const currentTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    try {
      const response = await axios.put(`http://localhost:5000/api/visitors/${id}`, { timeOut: currentTime });
